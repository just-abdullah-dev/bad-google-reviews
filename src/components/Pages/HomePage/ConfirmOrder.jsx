...
export default function ConfirmOrder({ 
  ...
  const totalAmount = data.noOfReviews * process.env.NEXT_PUBLIC_PRICE_PER_REVIEW;
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[600px] grid place-items-center">
        ...
        <div className=" w-full md:w-[400px] text-sm ">
            <div className=' grid gap-3'>
             ...
             <p className=" font-medium ">
               Google Map Link: <span className="text-black">{data?.googleMapLink}</span>
              </p>
              
              {data?.reviewSelectionOpt === 'specific' ?
               <p className=" font-medium">
               Specific Reviews Links: <span className="text-black">{data?.specificReviewLinks}</span>
              </p> :
              <Button
              type="button"
            ...
            <p className=" font-medium">
               No Of Reviews: <span className="text-black">{data?.noOfReviews}</span>
              </p>
        ...
        <div className="mt-6 flex w-full md:w-[400px] justify-between items-center">
          ...
          <Button className=' bg-blue-600 hover:bg-blue-500' onClick={handleConfirm}>Confirm</Button>
        </div>
      </DialogContent>
      {open && (
        <div 
